...
export class Bootstrap3FrameworkComponent {
    ...
    updateHelpBlock(value) {
        ...
        if (this.options.enableErrorState && value === 'INVALID' && this.formControl.errors &&
            ...
            this.options.helpBlock = Object.keys(this.formControl.errors).map(error => [error, Object.keys(this.formControl.errors[error]).map(errorParameter => errorParameter + ': ' +
                    this.formControl.errors[error][errorParameter]).join(', ')].filter(e => e).join(' - ')).join('<br>');
        }
    }
    /**
     * @return {?}
    ...
    updateArrayItems() {
        if (this.layoutNode.arrayItem && this.options.removable &&
            ...
            const /** @type {?} */ parentArray = JsonPointer.get(this.jsf.layout, this.layoutPointer, 0, -2);
    ...
    setTitle() {
        switch (this.layoutNode.type) {
            ...
            case 'authfieldset':
                ...
                return null;
            default:
                let /** @type {?} */ thisTitle = this.options.title || (isNaN(this.layoutNode.name) && this.layoutNode.name !== '-' ?
