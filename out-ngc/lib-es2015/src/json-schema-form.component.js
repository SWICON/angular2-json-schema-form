...
export class JsonSchemaFormComponent {
     ...
     * @param {?} sanitizer
     */
    constructor(frameworkLibrary, widgetLibrary, jsf, sanitizer) {
    ...
    initializeForm() {
        if (this.schema || this.layout || this.data ||
            ...
            let /** @type {?} */ loadExternalAssets = this.loadExternalAssets || false;
            let /** @type {?} */ framework = this.framework || 'default';
            ...
            if (alternateLayout) {
                JsonPointer.forEachDeep(alternateLayout, (value, pointer) => {
                    const /** @type {?} */ schemaPointer = pointer.replace(/\//g, '/properties/')
                        .replace(/\/properties\/items\/properties\//g, '/items/properties/')
                        .replace(/\/properties\/titleMap\/properties\//g, '/titleMap/properties/');
                    if (hasValue(value) && hasValue(pointer)) {
                        const /** @type {?} */ groupPointer = JsonPointer.parse(schemaPointer).slice(0, -2);
                        let /** @type {?} */ key = JsonPointer.toKey(schemaPointer);
                        let /** @type {?} */ itemPointer;
