...
export function buildSchemaFromData(data, requireAllFields = false, isRoot = true) {
    ...
    if (newSchema.type === 'object') {
        ...
        let /** @type {?} */ itemTypes = data.map(getFieldType).reduce((types, type) => types.concat(types.indexOf(type) === -1 ? type : []), []);
        const /** @type {?} */ buildSubSchemaFromData = (value) => buildSchemaFromData(value, requireAllFields, false);
        if (itemTypes.length === 1) {
            newSchema.items = data.map(buildSubSchemaFromData).reduce((combined, item) => Object.assign(combined, item), {});
        }
        else {
 ...
 * @return {?}
 */
export function getFromSchema(schema, dataPointer, returnContainer = false) {
 ...
 * @return {?}
 */
export function getSchemaReference(schema, reference, schemaRefLibrary = null, recursiveRefMap = null) {
    let /** @type {?} */ schemaPointer;
    let /** @type {?} */ newSchema;
 ...
 * @return {?}
 */
export function resolveRecursiveReferences(pointer, recursiveRefMap, arrayMap = new Map()) {
    let /** @type {?} */ genericPointer = JsonPointer.toGenericPointer(JsonPointer.compile(pointer), arrayMap);
    let /** @type {?} */ possibleReferences = true;
    let /** @type {?} */ previousPointerValues = [];
...
export function isInputRequired(schema, pointer) {
    ...
    let /** @type {?} */ listPointerArray = JsonPointer.parse(pointer);
    if (isArray(listPointerArray) && listPointerArray.length) {
        let /** @type {?} */ keyName = listPointerArray.pop();
        let /** @type {?} */ requiredList;
...
export function updateInputOptions(layoutNode, schema, jsf) {
    ...
    const /** @type {?} */ templatePointer = JsonPointer.get(jsf, ['dataMap', layoutNode.dataPointer, 'templatePointer']);
    ...
    if (templatePointer && schema.type !== 'array' && schema.type !== 'object') {
        let /** @type {?} */ layoutNodeValue = JsonPointer.getFirst([
            [jsf.defaultValues, layoutNode.dataPointer],
            [layoutNode, '/value'],
            [layoutNode, '/default']
        ]);
        let /** @type {?} */ templateValue = JsonPointer.get(jsf.formGroupTemplate, templatePointer + '/value/value');
